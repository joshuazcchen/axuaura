...
{
    sqlite3_stmt* s;
    std::string sql = "INSERT INTO shop_items (guild_id, type, role_id, name, desc, cost, data) VALUES (?, ?, ?, ?, ?, ?, ?)";
...
ShopItem shop_get(dpp::snowflake g_id, int item_id)
{
    ...
    sqlite3_stmt* s;
    std::string sql = "SELECT item_id, type, role_id, name, desc, cost, data, active FROM shop_items WHERE guild_id = ? AND item_id "
                      "= ?";
    if (sqlite3_prepare_v2(db_ptr, sql.c_str(), -1, &s, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(s, 1, std::to_string(g_id).c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_int(s, 2, item_id);
        if (sqlite3_step(s) == SQLITE_ROW) {
            ...
            if (desc)
                i.desc = reinterpret_cast<const char*>(desc);
            i.cost = sqlite3_column_int(s, 5);
            const unsigned char* data = sqlite3_column_text(s, 6);
            if (data)
                i.data = reinterpret_cast<const char*>(data);
...
std::vector<ShopItem> shop_get_sign(dpp::snowflake g_id, int sign)
{
    std::vector<ShopItem> items;
    sqlite3_stmt* s;
    std::string sql = "SELECT item_id, role_id, name, cost FROM shop_items WHERE guild_id = ? AND active = 1 AND ((? >= 0 AND cost "
