...
namespace db {
	void inv_add(dpp::snowflake g_id, dpp::snowflake u_id, int item_id) {
		sqlite3_stmt* s;
		std::string sql =
			"INSERT INTO inventory (guild_id, user_id, item_id, acquired, equipped) VALUES (?, ?, ?, ?, 0)";
	...
	std::vector<dpp::snowflake> inv_list(dpp::snowflake g_id, int item_id) {
		...
		if (sqlite3_prepare_v2(db_ptr, sql.c_str(), -1, &s, nullptr) == SQLITE_OK) {
			...
			while (sqlite3_step(s) == SQLITE_ROW) {
				...
				if (u_id_str) { users.push_back(std::stoull(u_id_str)); }
			}
			sqlite3_finalize(s);
		}
		return users;
	...
	std::vector<InvItem> inv_get_user(dpp::snowflake g_id, dpp::snowflake u_id) {
		...
		sqlite3_stmt* s;
		std::string sql =
			"SELECT i.inv_id, s.item_id, s.name, s.type, s.role_id, i.equipped, i.acquired FROM inventory i JOIN "
			"shop_items s ON i.item_id = s.item_id WHERE i.guild_id = ? AND i.user_id = ?";
		if (sqlite3_prepare_v2(db_ptr, sql.c_str(), -1, &s, nullptr) == SQLITE_OK) {
			sqlite3_bind_text(s, 1, std::to_string(g_id).c_str(), -1, SQLITE_TRANSIENT);
			sqlite3_bind_text(s, 2, std::to_string(u_id).c_str(), -1, SQLITE_TRANSIENT);
			while (sqlite3_step(s) == SQLITE_ROW) {
	...
	double inv_xp_mult(dpp::snowflake g_id, dpp::snowflake u_id) {
		sqlite3_stmt* s;
		double max_mult = 1.0;
		std::string sql = "SELECT s.data FROM inventory i JOIN shop_items s ON i.item_id = s.item_id WHERE i.guild_id "
						  "= ? AND i.user_id "
						  "= ? AND i.equipped = 1 AND s.type = 'xp_boost'";
		if (sqlite3_prepare_v2(db_ptr, sql.c_str(), -1, &s, nullptr) == SQLITE_OK) {
			sqlite3_bind_text(s, 1, std::to_string(g_id).c_str(), -1, SQLITE_TRANSIENT);
			sqlite3_bind_text(s, 2, std::to_string(u_id).c_str(), -1, SQLITE_TRANSIENT);
			while (sqlite3_step(s) == SQLITE_ROW) {
