...
std::vector<InvItem> inv_get_user(dpp::snowflake g_id, dpp::snowflake u_id)
{
    ...
    sqlite3_stmt* s;
    std::string sql = "SELECT i.inv_id, s.item_id, s.name, s.type, s.role_id, i.equipped, i.acquired FROM inventory i JOIN "
...
double inv_xp_mult(dpp::snowflake g_id, dpp::snowflake u_id)
{
    sqlite3_stmt* s;
    double max_mult = 1.0;
    std::string sql = "SELECT s.data FROM inventory i JOIN shop_items s ON i.item_id = s.item_id WHERE i.guild_id = ? AND i.user_id "
