...
namespace image {
	...
	void draw_inv_placard(Magick::Image& bg, const Magick::Image& tmpl, int px, int py, const db::InvItem& item) {
		...
		bg.fontPointsize(30);
		bg.fillColor(item.equipped ? "rgba(140,220,140,1)" : "rgba(200,180,120,1)");
		if (!sub.empty()) bg.draw(Magick::DrawableText(cx, py + 115, sub));
		if (item.equipped) {
			Magick::TypeMetric m;
			bg.fontTypeMetrics(sub.empty() ? name : sub, &m);
			bg.fontPointsize(26);
			bg.fillColor("rgba(140,220,140,1)");
			bg.draw(Magick::DrawableText(cx + (sub.empty() ? m.textWidth() + 8 : 0), py + 115, " [equipped]"));
		}
	}
