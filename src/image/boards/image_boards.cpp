...
namespace image {
    ...
    struct PodiumSpec { int cx; int top_y; int font_large; int font_small; };
    ...
    static void draw_centered(Magick::Image& img, const std::string& txt,
                               int cx, int y, int ptsize, const std::string& color) {
    ...
    static std::string render_board(const std::string& bg_file,
    ...
    std::string img_gen_auraboard(const std::vector<BoardEntry>& entries, bool is_bottom, int total_aura) {
        try {
            std::string file = is_bottom ? "assets/leaderboards/evil_board.png"
                                         : "assets/leaderboards/good_board.png";
