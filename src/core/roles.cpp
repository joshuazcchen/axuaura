...
void role_sync(dpp::cluster& bot)
{
    ...
    std::vector<std::pair<dpp::snowflake, int>> r_i_pairs = { { conf.leader_role, leader_id }, { conf.num2_role, num2_id },
        { conf.num3_role, num3_id }, { conf.loser_role, loser_id },
        { conf.bot2_role, bot2_id }, { conf.bot3_role, bot3_id } };
    ...
    for (auto const& [r_id, i_id] : r_i_pairs) {
        ...
        for (auto const& u_id : cur) {
            if (targets.count(u_id) && targets.at(u_id) == i_id)
                continue;
