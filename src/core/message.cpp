	...
	void handle(const dpp::message_create_t& event, dpp::cluster& bot) {
		...
		if (time_now - time_prev >= conf.xp_cooldown) {
			...
			if (xp_now >= xp_requ) {
				...
				bot.message_create(dpp::message(conf.lvl_ch, "<@" + std::to_string(user_id) + "> reached **level " +
		...
		if (dis(gen) <= conf.aurachancegain) { db::add_aura(event.msg.guild_id, user_id, conf.aurapassiveamt); }
			...
			if (!items.empty()) {
				...
				if (db::inv_has(event.msg.guild_id, user_id, picked_id)) {
					...
					bot.message_create(dpp::message(event.msg.channel_id,
													"<@" + std::to_string(user_id) + "> found **" + picked->name +
					...
					bot.message_create(dpp::message(
						...
						"<@" + std::to_string(user_id) + "> is lucky as hell and stumbled across a **" + picked->name +
		...
		if (std::regex_search(content, match, url_r)) {
			...
			for (const auto& attachment : event.msg.attachments) {
				if (attachment.filename.size() >= 4 &&
					(attachment.filename.substr(attachment.filename.size() - 4) == ".gif" ||
					 attachment.filename.substr(attachment.filename.size() - 4) == ".png" ||
					 attachment.filename.substr(attachment.filename.size() - 4) == ".jpg")) {
					if (ch) {
				...
				if (!(perm & dpp::p_embed_links)) {
					...
					bot.start_timer(
						[&bot, msg_id, ch_id, url, user_sf, g_id, conf](dpp::timer t) {
							bot.message_get(
								msg_id, ch_id,
								[&bot, ch_id, msg_id, url, user_sf, g_id,
								 conf](const dpp::confirmation_callback_t& res) {
