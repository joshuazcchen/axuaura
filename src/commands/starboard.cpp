...
namespace commands {
	...
	dpp::slashcommand starboard_admin_def(dpp::cluster& bot) {
		return dpp::slashcommand("starboard", "manage the starboard", bot.me.id)
			...
			.add_option(
					dpp::command_option(dpp::co_sub_command, "add", "add a channel for starboard")
					.add_option(dpp::command_option(dpp::co_channel, "channel", "channel", true)))
			.add_option(
					dpp::command_option(dpp::co_sub_command, "remove", "remove a channel")
					.add_option(dpp::command_option(dpp::co_channel, "channel", "channel to remove", true)))
			.add_option(
					dpp::command_option(dpp::co_sub_command, "list", "list channels"));
	}
	...
	void handle_starboard_admin(const dpp::slashcommand_t& event, dpp::cluster& bot) {
		...
		if (sub == "add") {
			...
			event.reply(dpp::message("added <#" + std::to_string(ch_id) + ">.")
					.set_flags(dpp::m_ephemeral));

		} else if (sub == "remove") {
			dpp::snowflake ch_id = std::get<dpp::snowflake>(event.get_parameter("channel"));
			db::sb_remove_channel(g_id, ch_id);
			event.reply(dpp::message("removed <#" + std::to_string(ch_id) + ">.")
					.set_flags(dpp::m_ephemeral));
			...
			event.reply(dpp::message(out).set_flags(dpp::m_ephemeral)
	...
	void handle_starboard_ctx(const dpp::message_context_menu_t& event, dpp::cluster& bot) {
		...
		if (!db::sb_is_allowed(g_id, ch_id)) {
			event.reply(dpp::message("this channel isn't set up for starboard nominations.")
					.set_flags(dpp::m_ephemeral));
			return;
		}
		...
		try { sb_ch = std::stoull(sb_ch_str); } catch (...) {}
		if (sb_ch == 0) {
		...
		pos_btn.set_type(dpp::cot_button).set_id(pos_id)
			.set_label("positive (" + std::to_string(pos_cost) + " aura)")
			.set_style(dpp::cos_success);

		dpp::component neg_btn;
		neg_btn.set_type(dpp::cot_button).set_id(neg_id)
	...
	void handle_starboard_button(const dpp::button_click_t& event, dpp::cluster& bot) {
		...
		dpp::snowflake sb_ch = 0;
		try { sb_ch = std::stoull(db::get_setting_str(g_id, "sb_channel", "0")); } catch (...) {}
		if (sb_ch == 0) {
		...
		if (aura < cost) {
			event.reply(dpp::message("not enough aura (you have **" + std::to_string(aura) +
		...
		bot.message_get(src_msg_id, src_ch_id,
