...
namespace commands {
	dpp::slashcommand bet_def(dpp::cluster& bot) {
		return dpp::slashcommand("bet", "bet aura", bot.me.id)
			...
			.add_option(
				dpp::command_option(dpp::co_sub_command, "start", "start bet")
	...
	void handle_bet(const dpp::slashcommand_t& event, dpp::cluster& bot) {
		...
		if (scmd == "place") {
			...
			if (aura < amt) { event.reply(dpp::message("too broke for this").set_flags(dpp::m_ephemeral)); }
			...
			event.reply(
				dpp::message("placed bet of " + std::to_string(amt) + " on " + side).set_flags(dpp::m_ephemeral));
		} else if (scmd == "list") {
			...
			db::p_end_poll(p_id);
			event.reply(win + " won, total of " + std::to_string(pot) + " paid out to " + std::to_string(count) +
						" winners");
		}
	}
} // namespace commands
