...
void handle_duel(const dpp::slashcommand_t& event, dpp::cluster& bot)
{
    ...
    if (cmd.name == "challenge") {
        ...
        // in the interest of not taking someones aura away before the duel is accepted im gonna just make it so that
        // the database stores that info.
        if (bet < 0 && u_aura < 0) {
        ...
        event.reply(
            dpp::message("<@" + std::to_string(target) + ">, hey, <@" + std::to_string(user_id) + "> thinks they're better than you. wanna try to prove them wrong? they're betting **" + std::to_string(bet) + "**.\n\naccept using /duel accept\n-# note that accepting a duel cancels your outgoing duels.")
        ...
        new dpp::oneshot_timer(&bot, 120, [&bot, user_id, target, ch_id, g_id](dpp::timer t) {
            ...
            if (issue_time != -1 && (std::time(nullptr) - issue_time >= 119)) {
                ...
                bot.message_create(dpp::message(
                    ch_id, "<@" + std::to_string(user_id) + ">'s duel to <@" + std::to_string(target) + "> expired"));
            }
        });
