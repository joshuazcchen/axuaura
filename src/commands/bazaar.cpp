...
void handle_bazaar(const dpp::slashcommand_t& event, dpp::cluster& bot)
{
    ...
    if (sub == "list") {
        ...
        if ((item.cost >= 0 && aura >= item.cost) || (item.cost < 0 && aura <= item.cost)) {
            ...
            event.reply(
                dpp::message("purchased and equipped **" + item.name + "**. manage your inventory using `/inventory`.")
        ...
        if (subcmd.name == "add") {
            ...
            for (auto& i : items) {
                std::string display = (i.type == "role") ? "<@&" + std::to_string(i.role_id) + ">" : i.name;
                out += std::to_string(i.item_id) + " " + display + " " + std::to_string(i.cost) + " aura" + (db::shop_state(g_id, i.item_id, "active") == 1 ? "(ACTIVE) " : "(INACTIVE) ") + (db::shop_state(g_id, i.item_id, "obtainable") == 1 ? "(OBTAINABLE)" : "(UNOBTAINABLE)");
                if (!i.desc.empty())
                    out += " " + i.desc + "*";
                out += "\n";
            }
