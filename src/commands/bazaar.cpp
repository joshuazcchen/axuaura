...
namespace commands {
	...
	void handle_bazaar(const dpp::slashcommand_t& event, dpp::cluster& bot) {
		...
		if (sub == "list") {
			...
			if (start_idx > items_full.size()) { start_idx = items_full.size(); }
			...
			for (auto& i : items) {
				...
				out += "**id: " + std::to_string(i.item_id) + "**, " + display + ", **price (tax inclusive): ** " +
					   std::to_string(i.cost) + " aura\n";
				if (!i.desc.empty()) out += "> *" + i.desc + "*\n";
			}
			out += "\n-# page " + std::to_string(++page) + " of " +
				   std::to_string((items_full.size() + config::BAZAAR_PGSZ) / config::BAZAAR_PGSZ);
			...
			if ((item.cost >= 0 && aura >= item.cost) || (item.cost < 0 && aura <= item.cost)) {
				...
				event.reply(dpp::message("purchased and equipped **" + item.name +
										 "**. manage your inventory using `/inventory`.")
